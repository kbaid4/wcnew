...
const CreateTaskPage = () => {
  ...
  useEffect(() => {
    async function fetchSuppliers() {
      ...
      // 3. Get ALL invited suppliers, regardless of signup status
      let allInvitedSuppliers = [];
      if (eventId) {
        const { data: invites, error: invitesError } = await supabase
          .from('invites')
          .select('supplier_email, created_at, status')
          .eq('event_id', eventId);
          
        if (!invitesError && Array.isArray(invites)) {
          // Get all registered emails to check registration status
          const registeredEmails = allSuppliers?.map(s => s.email.toLowerCase()) || [];
          
          // Process all invites, but mark which ones are registered
          allInvitedSuppliers = invites.map(invite => {
            const email = invite.supplier_email.toLowerCase();
            const isRegistered = registeredEmails.includes(email);
            
            // Find the full profile if registered
            const registeredProfile = isRegistered ? 
              allSuppliers.find(s => s.email.toLowerCase() === email) : null;
            
            return {
              id: isRegistered && registeredProfile ? 
                registeredProfile.id : // Use real ID if registered
                `invite_${invite.supplier_email}`, // Create a temporary ID otherwise
              email: invite.supplier_email,
              created_at: invite.created_at,
              status: invite.status,
              isInvited: true,
              isRegistered: isRegistered,
              full_name: registeredProfile?.full_name || null,
              company_name: registeredProfile?.company_name || null
            };
          });
        }
      }
    ...
    try {
      ...
      if (taskData.searchedSupplier) {
        ...
        if (!supplierObj) {
          ...
          // Check if it's an invited supplier (ID starts with "invite_")
          if (!supplierObj && taskData.searchedSupplier.startsWith('invite_')) {
            const invitedSupplier = invitedSuppliers.find(s => s.id === taskData.searchedSupplier);
            if (invitedSupplier) {
              supplierObj = { email: invitedSupplier.email };
            }
          }
        }
        ...
        if (supplierEmail) {
          ...
          try {
            const notificationData = {
              ...
              content: `You assigned a new task for this event.`  // Using 'content' instead of 'message'
            };
            ...
            if (notifErr.details) console.error('Error details:', notifErr.details);
            if (notifErr.hint) console.error('Hint:', notifErr.hint);
          }
    ...
    return (
      <option key={supplier.id} value={supplier.id}>
        {serviceType ? `${displayName} (${serviceType})` : `${displayName} (No Service Type)`}
                    ...
                    {mySuppliers.length === 0 && signedUpSuppliers.length === 0 && invitedSuppliers.length === 0 ? (
                      <option disabled>No suppliers found. Please add suppliers to this event first.</option>
                    ) : (
